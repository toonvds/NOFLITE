...
def ganitelin(train_x, train_t, train_y, test_x, parameters):
  ...
  """
    ...
    ## Structure
    # 1. Generator
    Y_tilde = generator(X, T, Y) # Y_tilde_logit
    ...
    # 2. Generator loss
    G_loss_GAN = -D_loss
    G_loss_Factual = tf.reduce_mean(tf.compat.v1.losses.mean_squared_error(
        labels=Y, predictions=(T * tf.reshape(Y_tilde[:, 1], [-1, 1]) + \
                          (1. - T) * tf.reshape(Y_tilde[:, 0], [-1, 1]))))

    G_loss = G_loss_Factual + alpha * G_loss_GAN

    # 3. Inference loss
    I_loss1 = tf.reduce_mean(tf.compat.v1.losses.mean_squared_error(
        labels=(T) * Y + (1 - T) * tf.reshape(Y_tilde[:, 1], [-1, 1]), predictions=tf.reshape(Y_hat[:, 1], [-1, 1])))
    I_loss2 = tf.reduce_mean(tf.compat.v1.losses.mean_squared_error(
        labels=(1 - T) * Y + (T) * tf.reshape(Y_tilde[:, 0], [-1, 1]), predictions=tf.reshape(Y_hat[:, 0], [-1, 1])))
